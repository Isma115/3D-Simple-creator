...
En modo puntos, `Mayus + click` va encadenando puntos de referencia usando `state.pathPoints` para crear lineas consecutivas y aprovechar la deteccion de bucles/caras ya existente. El arrastre de un vertice sigue disponible, pero ahora requiere `Alt + Mayus` para no interferir con la construccion por referencias. Ese arrastre desplaza el vertice a lo largo del eje dominante de la camara y actualiza su posicion junto con las lineas conectadas (no mueve puntos que vienen de bloques).
